...
const Certificate = () => {
	...
	const {
		data: detailKuitansi,
		isLoading: isDetailKuitansiLoading,
		refetch,
	...
	const columnDetailKuitansi = [
		...
		{
			name: "NTPN",
			cell: (row) => <span className="ff-numerals">{row?.ntpn ?? "-"}</span>,
		},
		{
			name: "Tanggal Setor",
			cell: (row) => (
			...
			cell: (row) => (
				<div className="d-flex justify-content-around">
					<Button
						...
						onClick={() => {
							navigate(
								`/kuitansi/${selectedCertificate?.jenis_karantina}_${row.id}/edit`
							);
						}}
					>
						<i style={{ cursor: "pointer" }} className="ri-edit-line " />
					</Button>
					<Button
						type="button"
						color="danger"
						...
						className="m-1"
						onClick={() =>
							setConfirmDelete(!confirmDelete) & setIdKuitansiDelete(row)
						}
					>
	...
	const columnDetail = [
		...
		},
		{
			...
			cell: (row) => (
				<span className="ff-numerals">
					{row?.volume_netto.toLocaleString() ?? "-"}
				</span>
			),
		},
		{
		...
		},
		{
			...
			cell: (row) => (
				<span className="ff-numerals">
					{row?.volume_bruto?.toLocaleString() ?? "-"}
				</span>
			),
		},
		{
		...
		},
		{
			...
			cell: (row) => (
				<span className="ff-numerals">
					{row?.volume_lain.toLocaleString() ?? "-"}
				</span>
			),
		},
		{
	...
	const filterData = (text) => {
		if (text != "") {
			const balikan = listData.filter(
				(item) =>
					(item.no_aju &&
						item.no_aju.toLowerCase().includes(text.toLowerCase())) |
					(item.jenis_karantina &&
						item.jenis_karantina.toLowerCase().includes(text.toLowerCase())) |
					(item.no_dok_permohonan &&
						item.no_dok_permohonan.toLowerCase().includes(text.toLowerCase())) |
					(item.tgl_dok_permohonan &&
						item.tgl_dok_permohonan
							.toLowerCase()
							.includes(text.toLowerCase())) |
					(item.status_bayar &&
						item.status_bayar.toLowerCase().includes(text.toLowerCase())) |
					(item.nama_pemohon &&
						item.nama_pemohon.toLowerCase().includes(text.toLowerCase())) |
					(item.nama_pengirim &&
						item.nama_pengirim.toLowerCase().includes(text.toLowerCase()))
			);
	...
	return (
		<>
                ...
                >
                  Ikan
					...
					<Card>
						<Card.Header>
							<Card.Title as="h6">
								Nomor Dokumen: {selectedCertificate?.no_dok_permohonan}
							</Card.Title>
						</Card.Header>
						<Card.Body>
							<Tabs
							...
							>
								<Tab eventKey="home" title="Data Kuitansi">
									<Button
										style={{
											display: selectedCertificate?.id ? "block" : "none",
										}}
										type="button"
										color="secondary"
										className="me-1 my-2"
										onClick={() => {
											navigate(
												`/kuitansi/${selectedCertificate?.jenis_karantina}_${selectedCertificate?.id}/create`
											);
										}}
									>
										<i
								...
								<Tab eventKey="profile" title="Data Media Pembawa">
									...
									<small
										style={{
											display: selectedCertificate?.id ? "none" : "block",
										}}
									>
										*Mohon pilih permohonan diatas
									</small>
								</Tab>
							</Tabs>
						</Card.Body>
			...
			<Modal
				show={confirmDelete}
				onHide={() => setConfirmDelete(!confirmDelete)}
				animation={false}
			>
				...
				<Modal.Body className="pb-0">
					Kuitainsi nomor:{" "}
					<strong>{`${idKuitansiDelete?.nomor} akan dihapus.`}</strong>{" "}
					<p>Yakin ?</p>
				</Modal.Body>
				<Modal.Footer>
					<Button
					...
					<Button
						color="danger"
						onClick={() => deleteItem(idKuitansiDelete?.id)}
					>
						Hapus
					</Button>
				</Modal.Footer>
